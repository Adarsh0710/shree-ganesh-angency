...
export async function buildInvoicePDFBuffer({ user, client, invoice }) {
  return new Promise(async (resolve, reject) => {
    ...
    if (user.company?.logo) {
      const logoPath = path.join(projectRoot, 'uploads', user.company.logo);

      if (fs.existsSync(logoPath)) {
        try {
          doc.image(logoPath, leftX, y, { fit: [120, 60] });
          y += 70;
        } catch {
          // ignore bad image
        }
    ...
    doc
      ...
      .fontSize(16)
      .text(
        user.company?.name?.trim() || user.name || 'Shree Ganesh Agency',
        leftX,
        y
      );
    ...
    if (user.company?.gstin) {
      doc.text(`GSTIN: ${user.company.gstin}`);
      y += 12;
    }

    if (user.company?.phone) {
      doc.text(`M: ${user.company.phone}`);
      y += 12;
    }

    if (user.company?.email) {
      doc.text(`Email: ${user.company.email}`);
      y += 12;
    }

    if (user.company?.addressLines?.length) {
      user.company.addressLines.forEach((line) => {
        doc.text(line);
        y += 12;
      });
    }
    ...
    invoice.items.forEach((item, i) => {
      ...
      doc.text(formatINR(item.rate), 250, y);
      doc.text(item.quantity, 320, y);
      doc.text(`₹ ${formatINR(item.amount)}`, 380, y);
      y += 20;
    });
    ...
    doc
      .font('Helvetica-Bold')
      .text(`Total: ₹ ${formatINR(invoice.total)}`, 350, y);

    y += 20;

    doc
      .font('Helvetica')
      .text(`Amount Paid: ₹ ${formatINR(invoice.amountPaid || 0)}`);

    y += 12;

    doc.text(
      `Amount Due: ₹ ${formatINR(
    ...
    if (user.company?.upiId) {
      try {
        const qr = await QRCode.toBuffer(
          `upi://pay?pa=${user.company.upiId}&cu=INR`
        );
        doc.image(qr, 200, y + 20, { width: 100 });
      } catch {}
    }
    ...
    doc
      ...
      .text(invoice.notes || 'Thank you for your business!', leftX, y);
